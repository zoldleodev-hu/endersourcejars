...
public class JarCPH implements ICustomPacketHandler.IClientPacketHandler {
    @Override
    public void handlePacket(@NotNull PacketCustom packet, @NotNull Minecraft mc, ClientPacketListener listener) {
        switch (packet.getType()) {
            case JarNetwork.C_SOURCE_SYNC -> EnderStorageManager.instance(true).getStorage(Frequency.readFromPacket(packet), EnderSourceStorage.TYPE).setSource(packet.readInt());
